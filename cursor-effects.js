var cursoreffects = function(t) {
    ...
    return t.bubbleCursor = function(t) {
        ...
        let e, n, i, o, s = t && t.imageURL || "girl.png", // Replace with your PNG image URL
            h = t && t.element,
            c = h || document.body,
            l = window.innerWidth,
        ...
        function A() {
            if (u.matches) return console.log("This browser has prefers reduced motion turned on, so the cursor did not init"), !1;
            e = document.createElement("canvas"), n = e.getContext("2d"), e.style.top = "0px", e.style.left = "0px", e.style.pointerEvents = "none", h ? (e.style.position = "absolute", c.appendChild(e), e.width = c.clientWidth, e.height = c.clientHeight) : (e.style.position = "fixed", document.body.appendChild(e), e.width = l, e.height = a);
            let t = new Image(); // Create a new Image object
            t.src = s; // Set the image source to the PNG image URL
            t.onload = function() { // Wait for the image to load
                o = t; // Assign the loaded image to the o variable
                d = new x(o); // Create a single instance of x
        ...
        function p() {
            !function() {
              e.width = e.width;
              d.position.x = r.x + 10;
              d.position.y = r.y + 80; // subtract half of the image height to make it hang from the mouse
              let i = new v(0, 0);
              let o, s, h = new v(10 * -d.velocity.x, 10 * -d.velocity.y),
                c = new v((i.X + h.X) / 1, (i.Y + h.Y) / 1 + 50);
              d.velocity.x += 0.01 * c.X;
              d.velocity.y += 0.01 * c.Y;
              Math.abs(d.velocity.x) < 0.1 && Math.abs(d.velocity.y) < 0.1 && Math.abs(c.X) < 0.1 && Math.abs(c.Y) < 0.1 && (d.velocity.x = 0, d.velocity.y = 0);
              d.position.x += d.velocity.x;
              d.position.y += d.velocity.y;
              o = e.clientHeight;
              s = e.clientWidth;
              d.position.y >= o - 11 - 1 && (d.velocity.y > 0 && (d.velocity.y = 0.7 * -d.velocity.y), d.position.y = o - 11 - 1);
              d.position.x >= s - 11 && (d.velocity.x > 0 && (d.velocity.x = 0.7 * -d.velocity.x), d.position.x = s - 11 - 1);
              d.position.x < 0 && (d.velocity.x < 0 && (d.velocity.x = 0.7 * -d.velocity.x), d.position.x = 0);
              d.draw(n);
            }(), i = requestAnimationFrame(p);
          }
